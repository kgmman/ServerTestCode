...
bool CMainFrame::InitServer()
{
	...
	if( bind(m_hServerSocket, (SOCKADDR*)&servAddr, sizeof(servAddr)) == SOCKET_ERROR )
		return false;
	...
	m_hClientSocket = accept(m_hServerSocket, (SOCKADDR*)&clntAddr, &szClntAddr);

	if( m_hClientSocket == INVALID_SOCKET )
		return false;

	send(m_hClientSocket, message, sizeof(message), 0);
